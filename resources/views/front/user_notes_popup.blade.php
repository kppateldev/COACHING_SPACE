  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content modal-before-bg position-relative">
      ...
      <div class="modal-body p-0">
        ...
        {!! Form::open(['url'=>url('user-notes-submit'), 'id'=>'user-notes-submit']) !!}

        <input type="hidden" name="sessionID" value="{{ $session->id }}">

        <div class="form-group">
          <label class="form-label mb-1">User Notes</label>
          <textarea name="user_notes" class="form-control h-auto" rows="3">{{ $session->user_notes ?? '' }}</textarea>
        </div>
        <div class="modal-footer p-0 border-0 justify-content-center">
          <input type="submit" name="submit" class="btn btn-main-primary mw-btn-185" value="Update">
        </div>
        {!! Form::close() !!}
